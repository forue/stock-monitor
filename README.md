...
常驻进程 (monitor-daemon.py)
  ...
  ├── 场景分类 → 11种异动场景智能识别 (含涨停/跌停破板)
  ├── 涨停/跌停状态追踪 → 涨停后静默，破板时触发反转告警
  ├── 后告警抑制 → 上次告警后量小波动低时自动抑制
  ├── 阶梯式递增告警 → 按趋势/反转/时间衰减规则推送到 QQ
  └── 实盘辅助功能 → 主力资金流 / 五档盘口 / 量价背离（可独立开关）
```

## 🎯 实盘辅助功能

独立于异动监控，各自可开关（`config.json` → `real_time_features`），默认关闭、开启才发起对应请求（保护免费源频控）。三个功能均**单只串行请求**，不违反"不可并行批量读取"约束；任一失败不阻断主循环。

### 功能一：主力资金流向监控（`fund_flow`）
数据源：东方财富 `fflow/kline`。识别主力吸筹/出货、量价背离（价涨资金流出=诱多、价跌资金流入=逆势吸筹）。
```json
"fund_flow": { "enabled": false, "check_interval": 300,
               "net_inflow_th": 1000000, "net_outflow_th": -1000000, "ratio_th": 0.05 }
```

### 功能二：五档盘口监控（`order_book`）
数据源：腾讯行情买卖五档。识别下方托盘/上方压盘（委比失衡）、涨停封板/开板。
```json
"order_book": { "enabled": false, "check_interval": 120,
                "vi_ratio_high": 0.6, "vi_ratio_low": -0.6, "seal_qty_th": 50000 }
```

### 功能三：量价背离检测（`divergence`）
复用 DB 量价历史 + 可选 RSI 过滤。识别顶背离（价新高量萎缩，见顶预警）/ 底背离（价新低量回升，反弹关注）。
```json
"divergence": { "enabled": false, "check_interval": 300, "window": 5 }
```

完整设计见 `docs/FEATURES.md`。
...
| 涨停破板 | 🔓 | 涨停后开板，价格从涨停价下跌 | 出货信号 |
| 跌停破板 | 🔓 | 跌停后开板，价格从跌停价上涨 | 抄底信号 |
...
|------|--------------|------------|
| 上海主板 | 60xxxx | ±10% |
| 深圳主板 | 00xxxx, 001xxx, 002xxx, 003xxx | ±10% |
| 创业板 | 30xxxx | ±20% |
| 科创板 | 68xxxx | ±20% |
...
- **串行取数（设计约束）**：数据源按"一只股票取完再取下一只"的顺序逐只获取，**不做并行批量请求**。免费源会对同一 IP 的并发/高频请求做限流甚至封禁 IP，批量并发取数会显著提高触发风控的概率；主循环内已内置 1.5s 间隔降低请求频率。
...
| 11种场景分类 | 量价共振/急速拉升/跌势加速/反弹/回落/涨停破板/跌停破板等智能识别 |
| 涨停/跌停追踪 | 自动识别板块涨跌停幅度（主板10%/创业板20%/科创板20%），涨停后静默，破板时触发反转告警 |
| 后告警抑制 | 上次告警后量小波动低时自动抑制，减少噪音 |
| 多源降级 | 腾讯→东方财富→新浪→网易，自动切换 |
| 分时段量比 | 开盘/早盘/午后/收盘动态阈值，避免集合竞价误触 |
| 实盘辅助功能 | 主力资金流 / 五档盘口 / 量价背离，独立开关 |
| 配置热加载 | 修改 config.json 无需重启 |
